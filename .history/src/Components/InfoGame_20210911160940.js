...
class InfoGame extends Component {
  ...
  handlePlayGame = () => {
    const { totalWhilePlay, type } = this.props.game
    const typeWhilePlay = totalWhilePlay >= 3 && totalWhilePlay <= 10 ? "Xỉu" : "Tài"
    console.log(totalWhilePlay)
    this.props.randomXucSac()
    console.log("TypeWhile", typeWhilePlay, "Type", type)
